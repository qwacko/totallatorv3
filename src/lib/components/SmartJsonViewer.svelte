<script lang="ts">
	...
	function processStringContent(str: string): { type: 'json' | 'text'; content: any } {
	...
	function getValueInfo(value: any): { type: string; preview: string; isExpandable: boolean } {
		if (value === null) return { type: 'null', preview: 'null', isExpandable: false };
		if (value === undefined)
		...
		if (typeof value === 'string') {
			...
			if (processed.type === 'json') {
				return { type: 'nested-json', preview: 'Structured Content', isExpandable: true };
			}
			return {
				type: 'string',
		...
		if (typeof value === 'boolean')
			return { type: 'boolean', preview: value.toString(), isExpandable: false };

		if (Array.isArray(value)) {
			return { type: 'array', preview: `Array(${value.length})`, isExpandable: value.length > 0 };
		}
			...
			{#if isExpanded}
				{#each data as item, index}
					<div class="ml-2 border-l border-gray-200 pl-2 dark:border-gray-600">
						...
						<SmartJsonViewer data={item} level={level + 1} defaultExpanded={defaultExpanded} />
					</div>
				{/each}
			{/if}
		{:else}
						...
						{#if info.isExpandable}
							{#if info.type === 'nested-json' && typeof value === 'string'}
								...
								{#if processed.content.prefix}
									<div class="mt-1 whitespace-pre-wrap text-gray-600 dark:text-gray-400">
							...
							{:else if info.type === 'string' && typeof value === 'string'}
								...
								{#if isExpanded}
									<pre
										class="mt-1 overflow-x-auto rounded bg-gray-50 p-2 text-xs whitespace-pre-wrap dark:bg-gray-800">{value}</pre>
								{/if}
							{:else}
								<SmartJsonViewer data={value} level={level + 1} defaultExpanded={defaultExpanded} />
							{/if}
						{:else}
							<!-- Simple value -->
							<span
