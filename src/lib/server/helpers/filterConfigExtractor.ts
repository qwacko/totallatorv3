import type { TextFilterOptionsType } from '../db/actions/helpers/misc/processTextFilter';
import type { AutocompleteKey } from '$lib/schema/textFilterSchema';
...
function inferFilterMetadata(key: string, entityType: string): Partial<FilterMetadata> {
	...
	// Boolean flags
	if (['disabled:', 'active:', 'allowupdate:', 'cash:', 'networth:', 'nw:', 'file:', 'files:', 'note:', 'notes:', 'reminder:', 'reminders:'].includes(lowerKey)) {
		return {
			type: 'boolean',
	...
	// Number fields
	if (lowerKey.includes('min') || lowerKey.includes('max') || lowerKey.includes('count') || lowerKey.includes('total')) {
		return {
			type: 'number',
			description: generateDescription(key, entityType),
	...
	// Date fields
	if (lowerKey.includes('date') || lowerKey.includes('before') || lowerKey.includes('after') || lowerKey.includes('first') || lowerKey.includes('last')) {
		return {
			type: 'date',
			description: generateDescription(key, entityType),
