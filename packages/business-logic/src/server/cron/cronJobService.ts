...
export class CronJobService {
  ...
  private async syncJobDefinitionsToDatabase(): Promise<void> {
    try {
      ...
      console.warn('Cron tables not yet available, skipping job sync. This is normal during initial startup.');
      return;
    }
  ...
  private async startScheduler(): Promise<void> {
    let enabledJobs;
    try {
      enabledJobs = await this.db
        .select()
        .from(cronJob)
        .where(eq(cronJob.isEnabled, true));
  ...
  async triggerJob(
    ...
    try {
      ...
      const executionId = await this.executeJob(
        jobDefinition, 
        jobId, 
        'manual', 
        triggeredByUserId
  ...
  private async runJobExecution(
    ...
    try {
      ...
      status = error instanceof Error && error.message === 'Job execution timeout' ? 'timeout' : 'failed';
  ...
  private resolveScheduleExpression(schedule: string): string {
    return schedule
      ...
      .replace('${AUTOMATIC_FILTER_SCHEDULE}', process.env.AUTOMATIC_FILTER_SCHEDULE || '*/15 * * * *')
  ...
  async getExecutionHistory(filter: CronJobExecutionFilter = {}): Promise<{
    ...
    const [executions, totalCountResult] = await Promise.all([
      ...
      this.db
        .select({ count: cronJobExecution.id })
        .from(cronJobExecution)
        .where(whereClause)
    ]);
