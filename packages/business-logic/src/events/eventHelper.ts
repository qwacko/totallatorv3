import { getContextEventEmitter, type AppEvents } from '@totallator/context';
 ...
 */
export function emitEvent<T extends keyof AppEvents>(
  eventName: T,
  payload: AppEvents[T]
): void {
  try {
 ...
 */
export function emitMultipleEvents<T extends keyof AppEvents>(events: Array<{
  name: T;
  payload: AppEvents[T];
}>): void {
