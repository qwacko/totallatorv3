...
export interface HookBuilderConfig<GlobalContextType, RequestContextType> {
	...
	) => GlobalContextType | Promise<GlobalContextType>;
...
export class ContextHandler<GlobalContextType, RequestContextType> {
	private contextStorage = new AsyncLocalStorage<
		CombinedContext<GlobalContextType, RequestContextType>
	>();
	...
	getContext = (): CombinedContext<GlobalContextType, RequestContextType> => {
		if (!this.isInitialized) {
			throw new Error(
				'ContextHandler not initialized - call hookBuilder() first in your hooks.server.ts'
			);
		}
		...
		if (!context) {
			throw new Error(
				'Context not available - must be called within a request or standalone context'
			);
		}
		return context;
	};
	 ...
	 */
	hookBuilder = (
		config: HookBuilderConfig<GlobalContextType, RequestContextType>
	): HookBuilderResult<GlobalContextType, RequestContextType> => {
		this.isInitialized = true;

		const { initGlobalContext, createRequestContext, updateLocals, customResolve } = config;
