...
async function testQueryLogs() {
  ...
  try {
    ...
    const logger = createLogger(true, ['TRACE', 'DEBUG', 'INFO', 'WARN', 'ERROR'], 'test-query-123');
    ...
    serverLogger.warn({ code: 'SRV002', title: 'High memory usage detected', memory: '85%' });
    
    authLogger.info({ code: 'AUTH001', title: 'User authenticated', userId: 'user123' });
    authLogger.error({ code: 'AUTH002', title: 'Failed login attempt', ip: '192.168.1.100' });
    
    importLogger.debug({ code: 'IMP001', title: 'Processing import file', filename: 'data.csv' });
