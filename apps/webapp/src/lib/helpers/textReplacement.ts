...
export function findWordBoundariesAtCursor(
  ...
  if (cursorPosition < 0 || cursorPosition > text.length) {
    throw new Error(
      `Invalid cursor position: ${cursorPosition} for text length: ${text.length}`,
    );
  }
...
export function replaceWordAtCursor(
  ...
  if (text === undefined || text.length === 0) {
    return { newText: replacement, newCursorPosition: replacement.length };
  }
 ...
 */
export function getCurrentWordAtCursor(
  text: string,
  cursorPosition: number,
): string {
  const boundary = findWordBoundariesAtCursor(text, cursorPosition);
