...
describe("processConfigString", () => {
  it("should correctly replace substring operations for keys with spaces", () => {
    ...
    const configString =
      '{{substring (getProperty this "description text") 2 7}}';
  ...
  it("should correctly replace substring operations (Complex Keys)", () => {
    ...
    const configString =
      '{{substring (getProperty this "This_Has/N&Thing") 2 7}}';
    const result = processConfigString(configString, inputObject);
    expect(result).toBe("is is a");
  });
  ...
  it("should handle complex configurations", () => {
    ...
    const configString =
      "{{firstName}} {{lastName}} is {{multiply age 2}} years old";
    const result = processConfigString(configString, inputObject);
  ...
  it("should correctly handle conditional operations", () => {
    ...
    const configString =
      "{{#if (eq status 'active')}}{{firstName}}{{else}}{{lastName}}{{/if}}";
    ...
    const resultInactive = processConfigString(
      configStringInactive,
      inputObject,
    );
