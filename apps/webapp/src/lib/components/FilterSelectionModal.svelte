	...
	<div class="flex flex-col gap-1">
		{#if selectedKey !== undefined}
			<div class="flex flex-row items-center gap-6 self-start">
				<Button size="sm" color="alternative" onclick={() => (selectedKey = undefined)}>
					<ArrowLeftIcon /> Back
				</Button>
		...
		{#if selectedKey === undefined}
			...
			{#each filterKeys as currentKey}
				...
				{#if hasChildren}
					<Button size="sm" onclick={() => (selectedKey = currentKey)} class="flex flex-row gap-2">
						{currentKey}
						<ArrowRightIcon />
					</Button>
				{:else}
					<Button size="sm" href={filterToURL(filters[currentKey][0])} class="flex flex-row gap-2">
						{currentKey}
					</Button>
				{/if}
			{/each}
		{:else}
			...
			{#each currentFilterGroup as filter}
				<Button size="sm" href={filterToURL(filter)} class="flex flex-row gap-2">
					{filter.title}
				</Button>
			{/each}
		{/if}
