<script lang="ts">
  ...
  import {
    ...
    setLogConfiguration,
  } from "./logsDisplay.remote";

  let logConfigurations = $derived(await getLogConfigurations());

  const getUniqueArrayItems = <T extends Record<string, unknown>>(
    data: T[],
    key: keyof T,
  ) => {
    return data
  ...
  const domains = $derived(
    getUniqueArrayItems(logConfigurations.configurations, "domain"),
  );
  const actions = $derived(
    getUniqueArrayItems(logConfigurations.configurations, "action"),
  );
...
<Card class="max-w-none">
  ...
  <div class="flex items-center flex-row gap-3 mb-6">
    ...
    <Badge color="gray" class="text-xs"
      >{logConfigurations.configurations.length} configurations</Badge
    >
  </div>
  ...
  <Accordion>
    {#each destinations as destination}
      {@const destinationConfigurations =
        logConfigurations.configurations.filter(
      ...
      <AccordionItem>
        ...
        <!-- Domains Section -->
        <Accordion>
          {#each domains.sort((a,b) => a.toLocaleString().localeCompare(b.toLocaleString())) as domain}
                ...
                <div class="space-y-3 p-4">
                  {#each actions as action}
                    {@const matchingConfig =
                      logConfigurations.configurations.find(
                        (item) =>
                          item.domain === domain &&
                    ...
                    {#if matchingConfig}
                      ...
                      >
                        ...
                        <div class="flex flex-row gap-1 items-center">
                          {#each levels as level}
                            <form
                              {...setLogConfiguration.enhance(
                                async ({ submit }) => {
                                  await submit().updates(
                                    getLogConfigurations().withOverride(
                                      (config) => ({
                                        ...config,
                                        configurations:
                                          config.configurations.map((item) => {
                                            if (
                                              item.action === action &&
                                              item.destination ===
                                                destination &&
  ...
  <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
      ...
      <div>
        ...
        >
        <span class="text-blue-900 dark:text-blue-100"
          >{logConfigurations.configurations.length}</span
        >
      </div>
    </div>
  </div>
