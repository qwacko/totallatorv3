<script lang="ts">
  ...
  import type { BillDropdownType } from "@totallator/business-logic";
  import type { BillFilterSchemaType } from "@totallator/shared";
  ...
  let { filter = $bindable() }: { filter: BillFilterSchemaType | undefined } =
    $props();
  ...
  const itemToOption = (data: BillDropdownType[number]): SelectionType => {
    if ($billDropdownData) {
      const matchingItem = $billDropdownData.find(
