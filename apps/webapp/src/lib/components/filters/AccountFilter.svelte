<script lang="ts">
  ...
  import type { AccountDropdownType } from "@totallator/business-logic";
  import type { AccountFilterSchemaType } from "@totallator/shared";
  ...
  let {
    filter = $bindable(),
  }: { filter: AccountFilterSchemaType | undefined } = $props();
  ...
  const itemToOption = (data: AccountDropdownType[number]): SelectionType => {
    if ($accountDropdownData) {
      const matchingItem = $accountDropdownData.find(
  ...
  const itemToDisplay = (
    data: AccountDropdownType[number],
