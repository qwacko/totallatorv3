<script lang="ts">
  ...
  const {
    summary,
  }: { summary: AssociatedInfoDataType["journalSnapshots"][number] } = $props();
...
<div class="flex flex-col gap-2">
  <div class="flex flex-row items-center gap-2">
    <div class="flex">
      Journal Summary! ({formatDate(
        summary.earliest,
        $userInfoStore.dateFormat,
      )} - {formatDate(summary.latest, $userInfoStore.dateFormat)})
    </div>
  ...
  {#if expanded}
    ...
    <div class="flex flex-row items-center gap-2">
      ...
      {@render displayTextItem(
        "Positive Count : ",
        summary.positiveCount.toFixed(0),
      )}
    </div>
    <div class="flex flex-row items-center gap-2">
      ...
      {@render displayTextItem(
        "Negative Count : ",
        summary.negativeCount.toFixed(0),
      )}
    </div>
    <div class="flex flex-row items-center gap-2">
      {@render displayCurrencyItem(
        "Positive Sum (No Transfer) : ",
        summary.positiveSumNonTransfer,
      )}
      {@render displayTextItem(
    ...
    <div class="flex flex-row items-center gap-2">
      {@render displayCurrencyItem(
        "Negative Sum (No Transfer) : ",
        summary.negativeSumNonTransfer,
      )}
      {@render displayTextItem(
