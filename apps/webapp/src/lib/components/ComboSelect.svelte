  ...
  const defaultFilterItems: FilterFunction<T> = (items, search) => {
    return items
      .filter((item) => {
        ...
        const groupFound = display.group
          ? display.group.toLowerCase().includes(search)
          : false;
...
<div class="flex flex-col gap-2 {className} @container">
  ...
  {#if clearable && clearName}
    <input
      type="hidden"
      name={clearName}
      value={clearValue === true ? "true" : "false"}
    />
  {/if}
  ...
  <div class="@md:flex-row flex w-full flex-col gap-2">
    {#if !items}
      <div class="flex flex-row items-center gap-2">
        <Spinner size="4" /><P size="sm" weight="semibold" class="text-gray-400"
          >Loading...</P
        >
      </div>
    ...
    {:else}
      <div class="relative flex grow">
        ...
        />
        <div
          class="text-magnum-900 absolute right-2 top-1/2 z-10 -translate-y-1/2"
        >
          {#if $open}
        ...
        >
          {#if creatable}
            <div class="flex flex-row gap-1 items-center">
              ...
              <Button outline size="sm" onclick={setCreate}
                >{createDesc} - {$inputValue}</Button
              >
            </div>
          {:else}
            No results found
