import type { ChartProps } from "flowbite-svelte";

import type { JournalSummaryType } from "@totallator/business-logic";
import type { getCurrencyFormatter } from "@totallator/shared";
import type { SummaryItemDetailsType } from "@totallator/shared";
...
const filterData = ({
  ...
  return config.dateRange === "all"
    ? filterTrendData({
        data,
        dates: generateYearMonthsBetween(earliestDataDate, latestDataDate),
      })
    : config.dateRange === "last12"
      ...
      : config.dateRange === "toNow"
        ? filterTrendData({
            data,
            dates: generateYearMonthsBetween(
              earliestDataDate,
...
export const generatePIChartConfig = ({
  ...
  const usedData =
    config.dateRange === "all" || config.dateRange === "toNow"
      ...
      : config.includeTransfers
        ...
        : data.map((item) => ({
            ...item,
            sum: item.sum12MonthsWithoutTransfer,
          }));
  ...
  const sortedData = usedData
    ...
    .filter(
      (item) => !removeNull || !(item.id === null || item.id === undefined),
    );
