<script lang="ts">
  ...
  import type { ReportElementData } from "@totallator/business-logic";
  ...
  const locked = $derived(
    elementData && elementData.reportElementConfig.locked,
  );
      ...
      <div class="flex flex-row items-center gap-2">
        ...
        >
          {#snippet slotFooterContents({ activeFilter })}
            <Button onclick={() => (filterOpen[index] = false)} outline
              >Cancel</Button
            >
            <div class="grow"></div>
            <form
