import type { ReportConfigPartItemGroupingType } from "@totallator/shared";
import type { ReportConfigPartNegativeDisplayType } from "@totallator/shared";
...
export const groupedDataToTree = ({
  ...
  const useAbsoluteValues =
    negativeDisplay === "absolute" || negativeDisplay === "grouped";
  const isGrouped = negativeDisplay === "grouped";
  const negativeTotal = data.reduce(
  ...
  const maxDepth =
    groupings.indexOf("none") !== -1
      ? groupings.indexOf("none")
      : groupings.length;
  ...
  data.forEach((item) => {
    ...
    if (isGrouped) {
      currentLevel =
        item.value >= 0
          ? (root[0].children as TreeNode[])
          : (root[1].children as TreeNode[]); // Choose the "Positive" or "Negative" group based on the item value
    }

    let currentPath = isGrouped
      ? item.value >= 0
        ? "Positive"
        : "Negative"
      : "";
