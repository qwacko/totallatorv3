<script lang="ts" generics="T extends Record<string|number|symbol, unknown>">
  ...
  const booleanSetValue = $derived(
    setValue as Writable<string | boolean | undefined>,
  );
  const booleanClearValue = $derived(
    clearValue as Writable<string | boolean | undefined>,
  );
