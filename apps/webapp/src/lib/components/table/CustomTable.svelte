...
>
  ...
  const columnIdToTitle = (id: string) =>
    columns.find((item) => item.id === id)?.title || id;
  ...
  <div class="flex flex-row gap-2">
    {#if slotFilterModal}
      <Button
        ...
        class="flex p-2"
        color="light"
        onclick={() => (filterOpened = true)}
      >
        <FilterIcon />
      </Button>
      <Modal
        bind:open={filterOpened}
        size="lg"
    ...
    {#if onSortURL && sortOptions.length > 0}
      <OrderDropDown
        ...
        options={sortOptions}
        {onSortURL}
        optionToTitle={(option) =>
          columns.find((item) => item.sortKey === option)?.title || ""}
      />
    {/if}
    <TableColumnDropdown
      bind:shownColumns
        ...
        {#each data as row}
          ...
          <TableBodyRow class={isGrey ? "bg-primary-50" : ""}>
            {#if bulkSelection}
              <TableBodyCell>
                <ToggleFromArray
                  id={rowToId ? rowToId(row) : undefined}
                  bind:selectedIds
                />
              </TableBodyCell>
            {/if}
