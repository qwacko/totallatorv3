<script lang="ts" generics="OrderKeys extends string">
  ...
  const showDropdownCalculated = $derived(
    sortKey !== undefined || showDropdown,
  );
  ...
  const isNone = $derived(
    currentSort?.find((sort) => sort.field === sortKey) === undefined,
  );
