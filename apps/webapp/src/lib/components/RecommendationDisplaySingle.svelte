<script lang="ts">
  ...
  const updateAndSaveActive = $derived(
    loadingUpdateAndSave === recommendation.journalId,
  );
  const updateActive = $derived(loadingUpdate === recommendation.journalId);
</script>

{#snippet DisplayItemBadge(
  id: string,
...
{#snippet displayItem()}
  <div class="flex flex-row items-center gap-4">
    {#if recommendation.source === "llm"}
      ...
      <P size="2xl" class="p-2"
        >{((recommendation.llmConfidence || 0) * 100).toFixed(0)}%</P
      >
      <P>{recommendation.checkDescription}</P>
    {:else}
      <Badge color="green" class="mr-2">Similar</Badge>
      <P size="2xl" class="p-2"
        >{(recommendation.checkSimilarity * 100).toFixed(0)}%</P
      >
      <P>
  ...
  <div class="flex flex-row items-center gap-4">
    ...
    <P italic color="text-slate-600 dark:text-slate-600">
      {getCurrencyFormatter($currencyFormat).format(
        recommendation.journalAmount,
      )}
    </P>
  </div>
  ...
  {#if recommendation.source === "llm" && recommendation.llmReasoning}
    <div
      class="mt-2 border-l-2 border-blue-300 pl-3 text-sm italic text-slate-500"
    >
      <P size="sm" color="text-slate-500"
        >AI Reasoning: {recommendation.llmReasoning}</P
      >
    </div>
  {/if}
  <div class="item-center flex flex-row flex-wrap gap-2">
    {#if recommendation.payeeAccountId}
      {@render DisplayItemBadge(
        recommendation.payeeAccountId,
        $accountDropdownData,
      )}
    {/if}
    ...
    {#if recommendation.journalBillId}
      {@render DisplayItemBadge(
        recommendation.journalBillId,
        $billDropdownData,
      )}
    {/if}
    {#if recommendation.journalBudgetId}
      {@render DisplayItemBadge(
        recommendation.journalBudgetId,
        $budgetDropdownData,
      )}
    {/if}
    {#if recommendation.journalCategoryId}
      {@render DisplayItemBadge(
        recommendation.journalCategoryId,
        $categoryDropdownData,
      )}
    {/if}
  </div>
  ...
  <div class="flex w-full flex-row items-stretch gap-2">
    {#if update}
      ...
      >
        {#if updateActive}<Spinner class="flex" size="4" />{:else}<AddIcon
            class="flex"
          />{/if}
...
<Card size="xl" class="flex flex-col items-center gap-1">
  {@render displayItem()}
</Card>
