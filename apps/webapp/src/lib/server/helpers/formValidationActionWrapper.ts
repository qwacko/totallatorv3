...
type FormValidationWrapperParams<T extends ZodValidationSchema, RequireUser extends boolean> = {
	title: string;
	validation: T;
	action: (data: {
		data: Infer<T>;
...
export const formValidationActionWrapper = <T extends ZodValidationSchema, RequireUser extends boolean>(
	...
	return async (data: RequestEvent<Partial<Record<string, string>>, RouteId>) => {
