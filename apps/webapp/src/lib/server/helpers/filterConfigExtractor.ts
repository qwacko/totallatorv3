import type { TextFilterOptionsType } from "@totallator/business-logic";
 ...
 */
export function extractAutocompleteFromTextFilter<
...
>(
  ...
  filterArray.forEach((filterItem) => {
    const keys = Array.isArray(filterItem.key)
      ? filterItem.key
      : [filterItem.key];
    ...
    keys.forEach((key) => {
      ...
      if (
        isNegated ||
        keys.some((k) => k.startsWith("!") && k.slice(1) === baseKey)
      ) {
  ...
  keyMap.forEach((metadata, baseKey) => {
    ...
    const primaryKey =
      cleanKeys.sort((a, b) => a.length - b.length)[0] || baseKey;
