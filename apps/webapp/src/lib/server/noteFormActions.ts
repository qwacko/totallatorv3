...
export const noteFormActions = {
	addNoteJournalFilter: async (data: RequestEvent<Partial<Record<string, string>>, RouteId>) => {
		const form = await superValidate(data.request, zod4(createNoteJournalSchema));
		...
		try {
			...
			data.locals.global
				.logger('server')
				.error({ code: 'SRV_0004', title: 'Error Creating Note', error: e });
		}

		return { form };
	},
	addNote: async (data: RequestEvent<Partial<Record<string, string>>, RouteId>) => {
		...
		try {
			...
			data.locals.global
				.logger('server')
				.error({ code: 'SRV_0005', title: 'Error Creating Note', error: e });
		}
	},
	deleteNote: async (data: RequestEvent<Partial<Record<string, string>>, RouteId>) => {
