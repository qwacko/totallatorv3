...
export const actions = {
	update: async (data) => {
		...
		const form = await superValidate(data.request, zod4(updateReportElementSchema));
		...
		try {
			...
			data.locals.global.logger('reports').error({
				code: 'RPT_0006',
				title: 'Error Updating Report Element',
				error: e
			});
		}
	},
	...
	updateConfig: async (data) => {
		const formData = await superValidate(data.request, zod4(updateReportConfigurationSchema));
