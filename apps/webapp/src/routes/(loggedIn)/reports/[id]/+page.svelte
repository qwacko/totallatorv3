...
<PageLayout title={$reportData.title} size={$reportData.size}>
  {#snippet slotRight()}
    ...
    {:else}
      ...
      <Button color="primary" outline onclick={() => (edit = true)}
        ><EditIcon /></Button
      >
      <Button
        color="red"
      ...
      <ReportGridItem {cols} {rows} highlightOnHover={false}>
        <div class="flex h-full w-full flex-col gap-2">
          <div class="item-stretch flex grow flex-row gap-2">
            <div
              class="flex grow place-content-center content-center self-stretch"
            >
        ...
        {#snippet slotTitleLeft()}
          {#if edit && $reportData.reportElements[index].title}
            <Input
              class="mr-2"
              bind:value={$reportData.reportElements[index].title}
            />
          {:else if title}
