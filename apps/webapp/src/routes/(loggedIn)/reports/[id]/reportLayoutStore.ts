...
import type { ReportLayoutConfigType } from "@totallator/business-logic";
...
export const reportLayoutStore = (reportData: ReportLayoutConfigType) => {
  ...
  const addElement = () => {
    const maxOrder = get(reportLayoutStore).reportElements.reduce(
      (acc, item) => {
  ...
  const removeElement = (id: string) => {
    reportLayoutStore.update((reportData) => {
      return {
        ...reportData,
        reportElements: reportData.reportElements.filter(
          (item) => item.id !== id,
        ),
      };
    });
  ...
  const reset = () => {
    reportLayoutStore.set({
      ...reportData,
      reportElements: reportData.reportElements.sort(
        (a, b) => a.order - b.order,
      ),
    });
    fixOrderNumbers();
  ...
  const moveDown = (id: string) => {
    reportLayoutStore.update((reportData) => {
      ...
      const targetPosition = reportData.reportElements[index].order;
      if (targetPosition === reportData.reportElements.length)
        return reportData;
      return {
        ...reportData,
        reportElements: reportData.reportElements
