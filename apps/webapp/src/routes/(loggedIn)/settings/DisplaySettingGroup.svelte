<script lang="ts">
  ...
  const {
    data,
    rawData,
  }: { data: DisplaySettingItem[]; rawData: Record<string, any> | undefined } =
    $props();
</script>
          ...
          {#if item.options}
            <div class="flex flex-row">
              {#each item.options as option}
                <div
                  class="border-primary-500 flex border-b border-l border-t px-2 py-1 first:rounded-l-md last:rounded-r-md last:border-r"
