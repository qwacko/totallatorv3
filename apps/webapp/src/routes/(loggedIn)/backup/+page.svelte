<script lang="ts">
  ...
  import {
    customEnhance,
    defaultCustomEnhance,
  } from "$lib/helpers/customEnhance";
...
<PageLayout title="Backups" size="xl">
  {#snippet slotRight()}
    <Button
      href={urlGenerator({ address: "/(loggedIn)/backup/import" }).url}
      outline
      color="green"
    >
      Import
      ...
      <TableBody>
        {#each displayFiles as backup}
          <TableBodyRow>
            ...
            <TableBodyCell>
              {#if backup.compressed}<Badge color="blue">Compressed JSON</Badge
                >{:else}<Badge color="green">JSON</Badge>{/if}
            </TableBodyCell>
            <TableBodyCell>
              {#if backup.fileExists}<Badge>Exists</Badge>{:else}<Badge
                  color="red"
                >
                  Missing
                </Badge>{/if}
