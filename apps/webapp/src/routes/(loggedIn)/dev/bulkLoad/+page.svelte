...
<PageLayout title="Bulk Actions">
  ...
  <div class="flex flex-col gap-2">
    ...
    <div class="flex">
      Currently : {data.accountCount} Accounts Exist - {data.deletableAccountCount}
      can be deleted
    </div>
      ...
      {#each data.accountCreationOptions as currentOption}
        <form class="flex" method="POST" action="?/bulkAddAccounts" use:enhance>
          <input
            type="hidden"
            name="countIncome"
            value={currentOption.income}
          />
          <input
            type="hidden"
            name="countExpenses"
            value={currentOption.expense}
          />
          <input type="hidden" name="countAssets" value={currentOption.asset} />
          <input
            type="hidden"
            name="countLiabilities"
            value={currentOption.liability}
          />
