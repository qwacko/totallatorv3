<script lang="ts">
  ...
  const { value: importMappingIdValue, errors: importMappingIdErrors } =
    proxyForm.importMappingId;
  const { value: frequencyValue, errors: frequencyErrors } =
    proxyForm.frequency;
  const { value: accountIdValue, errors: accountIdErrors } =
    proxyForm.accountId;
  const { value: appAccessTokenValue, errors: appAccessTokenErrors } =
    proxyForm.appAccessToken;
  const { value: appIdValue, errors: appIdErrors } = proxyForm.appId;
  const { value: connectionIdValue, errors: connectionIdErrors } =
    proxyForm.connectionId;
  const { value: secretValue, errors: secretErrors } = proxyForm.secret;
  const { value: userAccessTokenValue, errors: userAccessTokenErrors } =
    proxyForm.userAccessToken;
  const { value: lookbackDaysValue, errors: lookbackDaysErrors } =
    proxyForm.lookbackDays;
  const { value: startDateValue, errors: startDateErrors } =
    proxyForm.startDate;
...
<div class="grid grid-cols-1 gap-2 md:grid-cols-2">
  ...
  {#if $importMappingDropdownData}
    <SelectInput
      ...
      items={$importMappingDropdownData.map((t) => ({
        name: t.title,
        value: t.id,
      }))}
...
<Accordion>
  <AccordionItem open={!closeAccordian}>
    {#snippet header()}Source Details{/snippet}
