...
<CustomHeader pageTitle="Cron Jobs" filterText={[]} pageNumber={1} numPages={1} />
		...
		>
			{#snippet slotCustomBodyCell({ row: currentRow, currentColumn })}
				{#if currentColumn.id === 'actions'}
					...
					<div class="flex flex-row justify-center">
						<ButtonGroup>
							...
							<Button class="p-2" color="primary" outline onclick={() => triggerJob(currentRow.id)}>
				...
				{:else if currentColumn.id === 'schedule'}
					<code class="rounded bg-gray-100 px-2 py-1 text-xs">{currentRow.schedule}</code>
