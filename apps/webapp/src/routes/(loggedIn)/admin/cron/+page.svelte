<script lang="ts">
  ...
  const { form: triggerForm, enhance: triggerEnhance } = superForm(
    data.triggerJobForm,
    {
  ...
  const { form: toggleForm, enhance: toggleEnhance } = superForm(
    data.toggleJobForm,
    {
    ...
    <div class="bg-white p-4 rounded-lg border">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p
            class="text-2xl font-bold {getSuccessRateColor(
      ...
      {#snippet slotCustomBodyCell({ row: currentRow, currentColumn })}
        {#if currentColumn.id === "actions"}
          ...
          >
            ...
            <input
              type="hidden"
              name="isEnabled"
              bind:value={$toggleForm.isEnabled}
            />
          </form>
        ...
        {:else if currentColumn.id === "lastRun"}
          {#if currentRow.latestExecution}
            {@const statusBadge = getStatusBadge(
              currentRow.latestExecution.status,
            )}
        ...
        {:else if currentColumn.id === "successRate"}
          ...
          <p class="text-xs text-gray-500">
            {currentRow.statistics.successfulExecutions}/{currentRow.statistics
              .totalExecutions}
          </p>
        {/if}
      {/snippet}
