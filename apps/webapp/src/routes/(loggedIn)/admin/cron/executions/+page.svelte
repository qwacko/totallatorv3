<script lang="ts">
	...
	import { CheckCircleOutline, CloseCircleOutline, ClockOutline } from 'flowbite-svelte-icons';
	...
	let shownColumns = $state(['status', 'jobName', 'startedAt', 'duration', 'triggeredBy', 'actions']);
		...
		<div class="bg-white p-4 rounded-lg border">
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-2xl font-bold text-green-600">{data.statistics.successRate}%</p>
				</div>
			</div>
		</div>
