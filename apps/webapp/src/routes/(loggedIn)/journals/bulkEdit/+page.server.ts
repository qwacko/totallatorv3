...
export const actions = {
	updateState: async ({ request, locals }) => {
		const form = await superValidate(request, zod4(updateStateActionValidation));
		...
		try {
			...
			locals.global.logger('journals').error({
				code: 'JRN_0012',
				title: 'Error Updating Journal State',
				error: e
			});
			redirect(
				302,
	...
	update: async ({ request, locals }) => {
		...
		if (!form.valid) {
			locals.global
				.logger('journals')
				.error({ code: 'JRN_0013', title: 'Update Form Is Not Valid' });
			redirect(302, form.data.currentPage);
		}
		...
		if (!parsedFilter.success) {
			locals.global
				.logger('journals')
				.error({ code: 'JRN_0014', title: 'Update Filter Is Not Valid' });
			redirect(302, form.data.currentPage);
		}
