...
<CustomHeader
  pageTitle="Clone {data.journals.count} Journal{data.journals.count > 1
    ? 's'
    : ''}"
...
<PageLayout title="Clone {data.journals.count} Journals">
  ...
  >
    ...
    <Button class="w-full" type="submit"
      >Clone {data.journals.count} Journals</Button
    >
    <Button disabled={!$tainted} class="w-full" onclick={() => form.reset()}
      >Reset</Button
    >
