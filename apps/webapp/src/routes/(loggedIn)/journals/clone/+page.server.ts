...
export const load = async (data) => {
	...
	const form = await superValidate(journalDataForForm, zod4(cloneJournalUpdateSchema));
...
export const actions = {
	clone: async ({ request, locals }) => {
		...
		if (!form.valid) {
			locals.global
				.logger('journals')
				.error({ code: 'JRN_0002', title: 'Clone Form Is Not Valid' });
		...
		if (!parsedFilter.success) {
			locals.global
				.logger('journals')
				.error({ code: 'JRN_0003', title: 'Clone Filter Is Not Valid' });
			redirect(302, form.data.currentPage);
		}
		...
		try {
			...
			locals.global
				.logger('journals')
				.error({ code: 'JRN_0004', title: 'Error Cloning Journals', error: e });
