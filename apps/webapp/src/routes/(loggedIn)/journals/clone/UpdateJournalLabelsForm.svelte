<script lang="ts">
	...
	import type { CloneJournalUpdateSchemaType, UpdateJournalSchemaType } from '@totallator/shared';
			...
			<div class="flex flex-col gap-6">
				<div class="flex flex-row gap-2">
					<ComboSelect
						...
						class=" flex grow"
					/>
					<Button onclick={addCurrentLabel} disabled={!enableAdd} class="h-min self-end">
						Add
					</Button>
					<Button
						...
						{#each $formData.addLabels as currentLabel}
							...
							{#if labelDetail}
								<div>
									...
									<Button outline size="xs" onclick={() => addLabelToggle(labelDetail.id)}>
										{labelDetail.title}
									</Button>
								</div>
							{/if}
						...
						<div class="flex flex-row flex-wrap gap-2">
							{#each $formData.removeLabels as currentLabel}
								...
								{#if labelDetail}
									...
									<Button size="xs" color="light" onclick={() => removeLabelToggle(labelDetail.id)}>
										{labelDetail.title}
									</Button>
								{/if}
							{/each}
