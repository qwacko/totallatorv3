<script lang="ts">
  ...
  const {
    ...
    form:
      | SuperForm<UpdateJournalSchemaType>
      | SuperForm<CloneJournalUpdateSchemaType>;
    allLabelIds: string[];
    ...
    if (labelId) {
      if ($formData.removeLabels) {
        if ($formData.removeLabels.includes(labelId)) {
          $formData.removeLabels = $formData.removeLabels.filter(
            (item) => item !== labelId,
          );
        } else {
    ...
    if (labelId) {
      if ($formData.addLabels) {
        if ($formData.addLabels.includes(labelId)) {
          $formData.addLabels = $formData.addLabels.filter(
            (item) => item !== labelId,
          );
        } else {
  ...
  const enableAdd = $derived(
    currentLabelId &&
      (!$formData.addLabels || !$formData.addLabels.includes(currentLabelId)),
  );
...
<div class="col-span-1 md:col-span-2">
  ...
  <Tabs>
              ...
              {#if labelDetail}
                <div>
                  ...
                  <Button
                    outline
                    size="xs"
                    onclick={() => addLabelToggle(labelDetail.id)}
                  >
                    {labelDetail.title}
                  </Button>
                </div>
    ...
    {#if allLabelIds.length > 0 || commonLabelIds.length > 0}
      <TabItem
        title="Remove {$formData.removeLabels &&
        $formData.removeLabels.length > 0
        ...
        <div class="flex flex-col gap-6">
          {#if allLabelIds.length > 0}
            {@const useAllLabelIds = allLabelIds.filter((item) =>
              $formData.removeLabels
                ? !$formData.removeLabels.includes(item)
                : true,
            )}
            <P class="flex text-sm font-semibold"
              >Labels On At Least One Journals</P
            >
                ...
                {#each useAllLabelIds as currentLabel}
                  ...
                  {#if labelDetail}
                    <Button
                      size="xs"
                      color="light"
                      onclick={() => removeLabelToggle(labelDetail.id)}
                    >
                      {labelDetail.title}
                    </Button>
                  {/if}
      ...
      <TabItem title="Clear {$formData.clearLabels ? '(Active)' : ''}">
        <Checkbox bind:checked={$formData.clearLabels}
          >Clear All Labels</Checkbox
        >
      ...
      <div class="flex flex-col gap-6">
        <div class="flex flex-row gap-2">
          ...
          <Button
            onclick={addSetLabel}
            disabled={!enableSet}
            class="h-min self-end">Add</Button
          >
          <Button
        ...
        <P class="flex text-sm font-semibold"
          >Labels To Set All Journals To Have</P
        >
