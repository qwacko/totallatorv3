...
export const load = async (data) => {
	...
	const modificationForm = await superValidate(change || {}, zod4(updateJournalSchema));
...
export const actions = {
	update: async (data) => {
		const form = await superValidate(data.request, zod4(filterFormSchemaWithPage));
		...
		try {
			...
			data.locals.global.logger('queries').error({
				code: 'QRY_0003',
				title: 'Reusable Filter Update Error',
				error: e
			});
	...
	updateChange: async (data) => {
		...
		try {
			...
			data.locals.global.logger('queries').error({
				code: 'QRY_0007',
				title: 'Reusable Filter Update Change Error',
				error: e
			});
