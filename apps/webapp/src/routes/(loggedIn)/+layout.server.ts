...
export const load = async ({ locals }) => {
  ...
  return {
    ...
    dataUpdated: {
      ...
      bills:
        (await tActions.bill.latestUpdate({ db: locals.db })).getTime() || 0,
      budgets:
        (await tActions.budget.latestUpdate({ db: locals.db })).getTime() || 0,
      tags: (await tActions.tag.latestUpdate({ db: locals.db })).getTime() || 0,
      labels:
        (await tActions.label.latestUpdate({ db: locals.db })).getTime() || 0,
      accounts:
        (await tActions.account.latestUpdate({ db: locals.db })).getTime() || 0,
      categories:
        (await tActions.category.latestUpdate({ db: locals.db })).getTime() ||
        0,
      importMappings:
        (
