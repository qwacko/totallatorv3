...
>
  ...
  <input
    type="hidden"
    name="configuration"
    value={JSON.stringify($detailFormData)}
  />
  {#if csvData}
    <input
      type="hidden"
      name="sampleData"
      value={JSON.stringify(csvData.slice(0, 5))}
    />
  {/if}
...
<Button onclick={() => formElement.requestSubmit()}>{submitButtonText}</Button>
