...
import { defaultJournalFilter, labelFilterSchema } from '@totallator/shared';
...
export const load = async (data) => {
	...
	const filterText = await tHelpers.label.filterToText({
		db,
		filter: pageInfo.searchParams || { page: 0, pageSize: 10 }
	});

	// Generate autocomplete configuration from server-side filter array
	const autocompleteKeys = extractAutocompleteFromTextFilter(tHelpers.label.filterArray, 'label');
