...
let cronQueue = null;
const resolveScheduleExpression = (schedule, env) => {
    return schedule.replace(/\$\{([^}]+)\}/g, (_match, envVar) => env[envVar] || '* * * * *');
};
export const setCronQueue = (queue) => {
    cronQueue = queue;
};
const getRepeatJobId = (cronJobId) => `cron-execute:${cronJobId}`;
export const syncCronDefinitionsAndSchedules = async () => {
    return await withSpan('worker.cron.sync-schedules', async (span) => {
        if (!cronQueue) {
        ...
        for (const jobDef of cronJobDefinitions) {
            await withSpan('worker.cron.sync-definition', async (definitionSpan) => {
                ...
                definitionSpan.setAttribute('totallator.cron.definition_name', jobDef.name);
                definitionSpan.setAttribute('totallator.cron.definition_exists', Boolean(existing));
                if (!existing) {
                ...
                attributes: {
                    'totallator.cron.definition_name': jobDef.name
                }
            });
        }
        const enabledJobs = await db.select().from(cronJob).where(eq(cronJob.isEnabled, true));
        span.setAttribute('totallator.cron.enabled_jobs', enabledJobs.length);
        const desiredRepeatJobs = new Map(enabledJobs.map((job) => [getRepeatJobId(job.id), job.schedule]));
        ...
        for (const enabled of enabledJobs) {
            await cronQueue.add('cron-execute', {
                type: 'cron-execute',
...
export const setCronJobEnabled = async (jobId, isEnabled, modifiedBy) => {
    return await withSpan('worker.cron.set-enabled', async () => {
        const context = await globalContext();
        await context.db
            .update(cronJob)
        ...
        tracerName: '@totallator/worker',
        attributes: {
            'totallator.cron.job_id': jobId,
            'totallator.cron.is_enabled': isEnabled,
            'totallator.cron.modified_by': modifiedBy
        }
    });
};
export const executeCronJobById = async (payload) => {
    return await withSpan('worker.cron.execute', async (span) => {
        const context = await globalContext();
        const db = context.db;
        const [jobRecord] = await db
            ...
            .limit(1);
        if (!jobRecord) {
            span.setAttribute('totallator.cron.job_found', false);
            return { success: false, message: 'Cron job not found' };
        }
        span.setAttribute('totallator.cron.job_found', true);
        ...
        span.setAttribute('totallator.cron.execution_id', execution.id);
        try {
            const executeDefinition = async () => standaloneContext({
                ...
                return await jobDefinition.job(runContext.global);
            });
        ...
        tracerName: '@totallator/worker',
        attributes: {
            'totallator.cron.payload_job_id': payload.cronJobId,
            'totallator.cron.triggered_by': payload.triggeredBy || 'manual',
            'totallator.cron.retry_count': payload.retryCount || 0
        }
    });
};
