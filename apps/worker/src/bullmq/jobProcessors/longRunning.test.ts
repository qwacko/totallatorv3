...
const standaloneContextMock = vi.fn(
	async (_requestContext: unknown, callback: () => Promise<unknown>) => callback()
);
const runTrackedLongProcessMock = vi.fn(async (params: any) =>
	params.run({
		jobId: 'job-1',
